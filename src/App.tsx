...
>
  ...
  <div className="absolute right-12 top-12 mono text-[9px] uppercase tracking-[.3em] text-primary/50">
    ...
    <section><SectionTitle eyebrow="Quick access" title="A few doors" /><div className="grid gap-3 sm:grid-cols-3">{[{v:'memories' as View, icon:ImageIcon, t:'Look back', d:'The gallery of us'}, {v:'voice' as View, icon:Mic2, t:'Hear me', d:'Voice notes, saved'}, {v:'wrapped' as View, icon:Sparkles, t:'Our year', d:'The numbers that matter'}].map(({v, icon:Icon, t, d}) => <button onClick={() => setView(v)} key={v} className="glass flex items-center gap-4 rounded-2xl p-4 text-left transition hover:border-primary/40"><Icon className="text-primary" size={20} /><div><p className="text-sm">{t}</p><p className="text-xs text-muted-foreground">{d}</p></div><ArrowRight size={15} className="ml-auto text-muted-foreground" /></button>)}</div></section>
  </div>;
}
