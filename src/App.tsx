...
function Topbar({ openMenu, onSearch }: { openMenu: () => void; onSearch: (value: string) => void }) {
  return <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-white/[.06] bg-[#0b1813]/85 px-5 backdrop-blur-xl md:px-10"><button className="md:hidden" onClick={openMenu} aria-label="Open menu"><Menu size={20} /></button><div className="relative hidden w-72 md:block"><Search className="absolute left-3 top-2.5 text-muted-foreground" size={16} /><input onChange={(e) => onSearch(e.target.value)} placeholder="Find a song or memory" className="w-full rounded-full border border-white/10 bg-white/[.04] py-2 pl-10 pr-3 text-xs outline-none placeholder:text-muted-foreground focus:border-primary/50" /></div><div className="flex items-center gap-5"><span className="mono hidden text-[10px] text-muted-foreground sm:inline">MADE FOR YOU</span><button aria-label="Notifications" className="text-muted-foreground hover:text-primary"><Bell size={17} /></button><div className="grid h-8 w-8 place-items-center rounded-full bg-accent/20 text-xs text-accent">P</div></div></header>;
}
